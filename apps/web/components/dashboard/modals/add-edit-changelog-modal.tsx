...
export function AddChangelogModal({
  ...
  return (
    <ResponsiveDialog open={forceOpen} onOpenChange={onForceOpenChange}>
      {forceOpen === undefined ? <ResponsiveDialogTrigger asChild>{trigger}</ResponsiveDialogTrigger> : null}
      <ResponsiveDialogContent className='max-h-[96%] max-w-full px-0 sm:max-w-2xl sm:p-8 lg:max-w-4xl'>
        <div className='flex w-full flex-col overflow-auto px-4 sm:gap-4 sm:px-1'>
          <ResponsiveDialogHeader>
            <ResponsiveDialogTitle>
              {isEdit ? `Edit ${PRODUCT.brandUpdates.singular}` : PRODUCT.brandUpdates.create}
            </ResponsiveDialogTitle>
            <ResponsiveDialogDescription>
              {sourceFeedbackId
                ? `Polish and publish a clean ${PRODUCT.brandUpdates.singular.toLowerCase()} from this completed request.`
                : `Share a polished ${PRODUCT.brandUpdates.singular.toLowerCase()} with your client.`}
            </ResponsiveDialogDescription>
          </ResponsiveDialogHeader>
