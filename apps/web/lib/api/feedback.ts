...
export const createFeedback = (
  ...
  withProjectAuth<FeedbackProps['Row']>(async (user, supabase, project, error) => {
    ...
    const feedbackRow = {
      ...
      pinned_url: data.pinned_url || null,
      pinned_label: data.pinned_label || null,
      priority: data.priority || 'normal',
      deadline_bucket: data.deadline_bucket || 'flexible',
    ...
    const { data: feedback, error: feedbackError } = await supabase
      .from('feedback')
      // @ts-expect-error studio request columns not in generated types yet
      .insert(feedbackRow)
      .select('*, user:user_id (*)')
      .single();
...
export const updateFeedbackByID = (
  ...
  withFeedbackAuth<FeedbackProps['Row']>(async (user, supabase, feedback, project, error) => {
    ...
    const extended = feedback as FeedbackWithUserInputProps;
    const updateRow = {
      ...
      pinned_url: data.pinned_url !== undefined ? data.pinned_url : extended.pinned_url,
      pinned_label: data.pinned_label !== undefined ? data.pinned_label : extended.pinned_label,
      priority: data.priority !== undefined ? data.priority : extended.priority,
      deadline_bucket: data.deadline_bucket !== undefined ? data.deadline_bucket : extended.deadline_bucket,
      assignee_id: data.assignee_id !== undefined ? data.assignee_id : extended.assignee_id,
      attachment_urls: data.attachment_urls !== undefined ? data.attachment_urls : extended.attachment_urls,
    };
    // Update feedback
