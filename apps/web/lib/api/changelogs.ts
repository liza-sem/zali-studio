...
import { ChangelogInputProps, ChangelogProps, ChangelogWithAuthorProps, ProfileProps, ProjectProps } from '@/lib/types';
import { formatRootUrl, isSlugValid } from '../utils';

// Create Changelog
export const createChangelog = (slug: string, data: ChangelogInputProps, cType: 'server' | 'route') =>
  withProjectAuth<ChangelogProps['Row']>(async (user, supabase, project, error) => {
    ...
    const changelogRow = {
      ...
      source_feedback_id: data.source_feedback_id || null,
    };
    const { data: changelog, error: changelogError } = await supabase
      .from('changelogs')
      // @ts-expect-error source_feedback_id not in generated types yet
      .insert(changelogRow)
      .select();
